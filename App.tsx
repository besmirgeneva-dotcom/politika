...
const StatGauge = ({ label, value, color, icon }: { label: string, value: number, color: string, icon: string }) => (
    <div className="flex flex-col gap-1 w-20 md:w-24">
        <div className="flex justify-between items-center text-[10px]">
            <span className="font-bold text-stone-400 flex items-center gap-1 truncate">{icon} {label}</span>
            ...
            <>
                ...
                <div className="absolute top-6 left-6 z-30 flex flex-col gap-2 pointer-events-none">
                    <div className="bg-stone-900/90 backdrop-blur-md p-3 rounded-xl border border-stone-700 shadow-2xl pointer-events-auto flex flex-row gap-4 items-center">
                        ...
                        <StatGauge label="Éco" value={gameState.economyHealth} color="bg-emerald-500" icon="💰" />
                        <StatGauge label="Armée" value={gameState.militaryPower} color="bg-blue-500" icon="⚔️" />
                        <StatGauge label="Pop" value={gameState.popularity} color="bg-purple-500" icon="❤️" />
                ...
                <div className="absolute top-6 right-6 z-30 flex flex-row items-center gap-2 pointer-events-none">
                    <div className="bg-stone-900/90 backdrop-blur-md p-2 pl-4 pr-2 rounded-full border border-stone-700 shadow-2xl pointer-events-auto flex items-center gap-3">
                        <div className="flex flex-col items-end mr-2">
                             <div className="bg-black/50 text-emerald-400 text-[9px] font-mono px-1.5 py-0.5 rounded border border-emerald-900/50 mb-0.5">TOKENS: {tokenCount}</div>
                        </div>
                        <div className="flex flex-col items-end cursor-pointer hover:opacity-80" onClick={() => setAppMode('portal_dashboard')}>
                            <span className="text-[9px] text-stone-400 uppercase font-bold tracking-widest">Président</span>
                        ...
                        <button onClick={() => setAppMode('portal_dashboard')} className="w-10 h-10 rounded-full bg-stone-800 hover:bg-red-600 text-white flex items-center justify-center transition-colors border border-stone-600 ml-2">☰</button>
                    </div>
                </div>
            </>
        )}
